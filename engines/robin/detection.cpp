...
uint32 RobinEngine::getFeatures() const {
	return _gameDescription->desc.flags;
}

const char *RobinEngine::getGameId() const {
	return _gameDescription->desc.gameid;
}


static const PlainGameDescriptor robinGames[] = {
	// Games
...
static const RobinGameDescription gameDescriptions[] = {
...
class RobinMetaEngine : public AdvancedMetaEngine {
public:
	RobinMetaEngine() : AdvancedMetaEngine(gameDescriptions, sizeof(RobinGameDescription), robinGames) {
	}
...
bool RobinMetaEngine::createInstance(OSystem *syst, Engine **engine, const ADGameDescription *gd) const {
	if (gd) {
		*engine = new RobinEngine(syst, (const RobinGameDescription *)gd);
		((RobinEngine *)*engine)->initGame((const RobinGameDescription *)gd);
	}
	return gd != 0;
}

bool RobinMetaEngine::hasFeature(MetaEngineFeature f) const {
	return
...
SaveStateDescriptor RobinMetaEngine::querySaveMetaInfos(const char *target, int slot) const {
...
void RobinMetaEngine::removeSaveState(const char *target, int slot) const {
	Common::String fileName = Common::String::format("%s-%02d.SAV", target, slot);
	g_system->getSavefileManager()->removeSavefile(fileName);
}
} // End of namespace Robin

#if PLUGIN_ENABLED_DYNAMIC(ROBIN)
REGISTER_PLUGIN_DYNAMIC(ROBIN, PLUGIN_TYPE_ENGINE, Robin::RobinMetaEngine);
#else
REGISTER_PLUGIN_STATIC(ROBIN, PLUGIN_TYPE_ENGINE, Robin::RobinMetaEngine);
#endif

namespace Robin {

void RobinEngine::initGame(const RobinGameDescription *gd) {
