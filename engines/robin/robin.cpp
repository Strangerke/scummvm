...
void RobinEngine::display16x16IndexedBuf(byte *buf, int index, Common::Point pos, bool transparent, bool updateScreen) {
...
void RobinEngine::display16x16Buf(byte *buf, Common::Point pos, bool transparent, bool updateScreen) {
	debugC(2, kDebugEngine, "display16x16Buf(buf, %d, %d)", pos.x, pos.y);

	display16x16IndexedBuf(buf, 0, pos, transparent, updateScreen);
}

void RobinEngine::SaveSurfaceUnderMouseCursor(byte *buf, Common::Point pos) {
	...
	for (int i = 0; i < 16; i++) {
		for (int j = 0; j < 16; j++) {
			buf[(i * 16) + j] = ((byte *)_mainSurface->pixels)[index2 + j];
		}
		index2 += 320;
	}
}

void RobinEngine::fill16x16Rect(byte col, Common::Point pos) {
	debugC(2, kDebugEngineTBC, "fill16x16Rect(%d, %d - %d)", col, pos.x, pos.y);

	int index = pos.x + (pos.y * 320);
	for (int i = 0; i < 16; i++) {
		for (int j = 0; j < 16; j++) {
			((byte *)_mainSurface->pixels)[index + j] = col;
		}
		index += 320;
	}
}

void RobinEngine::displayMousePointer() {
...
void RobinEngine::restoreSurfaceUnderMousePointer() {
...
void RobinEngine::displaySpeechBubble() {
	...
	for (int i = 0; i < 16; i++) {
		...
		for (int j = 0; j < var3; j++) {
			((byte *)_mainSurface->pixels)[tmpIndex + j] = 17;
		}
		index += 320;
	}
...
void RobinEngine::displaySpeechLine(int vgaIndex, byte *srcBuf, int &bufIndex) {
...
void RobinEngine::displaySpeech(byte *buf) {
...
void RobinEngine::initGameAreaDisplay() {
...
void RobinEngine::displayIsometricBlock(byte *buf, int var1, int posX, int posY, int var3) {
...
void RobinEngine::moveCharacters() {
...
void RobinEngine::prepareGameArea() {
...
void RobinEngine::resetSmallAnims() {
...
void RobinEngine::displaySmallIndexedAnim(byte index, byte subIndex) {
...
void RobinEngine::displaySmallAnims() {
...
int16 RobinEngine::sub16DD5(int x1, int y1, int x2, int y2) {
...
void RobinEngine::unselectInterfaceHotspots() {
...
void RobinEngine::displayCharacterStatBar(int8 type, int16 averagePosX, int8 score, int16 posY) {
...
void RobinEngine::displayString(byte *buf, Common::Point pos) {
...
void RobinEngine::displaySpeechBubbleTail(Common::Point displayPos) {
...
void RobinEngine::displaySpeechBubbleTailLine(Common::Point pos, int var2) {
...
byte RobinEngine::sub16799(int index, Common::Point param1) {
...
void RobinEngine::sub1693A_chooseDirections(int index) {
...
void RobinEngine::numberToString(int param1) {
...
byte RobinEngine::sub166DD(int index, Common::Point var1) {
...
byte RobinEngine::sub16722(int index, Common::Point var1) {
...
byte RobinEngine::sub16729(int index, Common::Point var1) {
...
byte RobinEngine::sub1675D(int index, Common::Point var1) {
...
void RobinEngine::checkNumericCode() {
...
void RobinEngine::handleGameMouseClick() {
...
void RobinEngine::checkClickOnGameArea(Common::Point pos) {
...
void RobinEngine::checkClickOnCharacter(Common::Point pos, bool &forceReturnFl) {
...
bool RobinEngine::isMouseOverHotspot(Common::Point mousePos, Common::Point hotspotPos) {
...
void RobinEngine::handleInterfaceHotspot(byte index, byte button) {
...
void RobinEngine::sub16685(int idx, Common::Point var1) {
...
byte RobinEngine::sub16675_moveCharacter(int idx, Common::Point var1) {
...
void RobinEngine::turnCharacter1(int index) {
...
void RobinEngine::turnCharacter2(int index) {
...
void RobinEngine::sub17224(byte type, byte index, int var4) {
...
void RobinEngine::sub17264(byte index, int var4) {
...
void RobinEngine::checkInterfaceActivationDelay() {
...
//		sub16064(var1, _scriptHandler->_byte15FFA);
		for (int i = 0; i < (var2 << 2); i++) {
			((byte *)_mainSurface->pixels)[index] = var1;
			((byte *)_mainSurface->pixels)[index + 1] = var1;
			((byte *)_mainSurface->pixels)[index + 2] = var1;
			index -= 320;
		}
	}
...
//		sub16064(23, 25 - _scriptHandler->_byte15FFA);
		...
		for (int i = 0; i < var2; i++) {
			((byte *)_mainSurface->pixels)[index] = 23;
			((byte *)_mainSurface->pixels)[index + 1] = 23;
			((byte *)_mainSurface->pixels)[index + 2] = 23;
			index -= 320;
		}
	}
...
byte *RobinEngine::loadRaw(Common::String filename, int filesize) {
...
void RobinEngine::setCurrentCharacter(int index) {
...
void RobinEngine::unselectInterfaceButton() {
...
Common::Event RobinEngine::_keyboard_getch() {
	warning("getch()");
...
bool RobinEngine::_keyboard_checkKeyboard() {
	return (_keyboard_nextIndex != _keyboard_oldIndex);
}

void RobinEngine::_keyboard_resetKeyboardBuffer() {
